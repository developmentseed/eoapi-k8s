...
eoAPI observability is implemented through two separate Helm charts:

### Main `eoapi` Chart (Core Monitoring)
Provides essential metrics collection infrastructure including Prometheus server, metrics-server, kube-state-metrics, node-exporter, and prometheus-adapter.

### `eoapi-observability` Chart (Visualization & Advanced Features)
Adds Grafana dashboards, visualization tools, and optional alerting/batch job metrics capabilities.
...
```bash
# 1. Deploy with basic monitoring enabled
helm install eoapi eoapi/eoapi --set monitoring.prometheus.enabled=true

# 2. Add Grafana dashboards
helm install eoapi-obs eoapi/eoapi-observability

# 3. Access Grafana (get password)
...
helm install eoapi eoapi/eoapi -f monitoring-values.yaml

# Deploy observability with custom settings
helm install eoapi-obs eoapi/eoapi-observability -f grafana-values.yaml
```
